 ...
 *
 * http://plantuml.com/state.html http://plantuml.com/plantuml
 */
public class ControlFlowGraphPlantUMLWriter {
    ...
    protected static final char[] PLANTUML_ENCODE_6_BIT = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_"
            .toCharArray();
    ...
    protected static void writeState(StringBuilder sb, Method method, BasicBlock basicBlock) {
        ...
        case TYPE_TRY_JSR:
        case TYPE_TRY_ECLIPSE:
            sb.append("state \"").append(basicBlock.getTypeName()).append(" : ").append(basicBlock.getIndex())
                    .append("\" as ").append(id).append("<<Reduced>>\n");
        ...
        case TYPE_GOTO:
            sb.append("state \"").append(basicBlock.getTypeName()).append(" : ").append(basicBlock.getIndex())
                    .append("\" as ").append(id).append("<<ToReduce>>\n");
            writeStateOffsets(sb, id, basicBlock);
            writeLineNumbers(sb, id, basicBlock);
            writeStatePredecessors(sb, id, basicBlock);
            writeStateCode(sb, id, method, basicBlock);
        ...
        case TYPE_JUMP:
            sb.append("state \"").append(basicBlock.getTypeName()).append(" : ").append(basicBlock.getIndex())
                    .append("\" as ").append(id).append("<<Synthetic>>\n");
    ...
    protected static void writeStateCode(StringBuilder sb, String id, Method method, BasicBlock basicBlock) {
        if ((method != null) && basicBlock.matchType(GROUP_CODE)
                && (basicBlock.getFromOffset() < basicBlock.getToOffset())) {
        ...
        case TYPE_SWITCH_DECLARATION:
            ...
            for (SwitchCase switchCase : basicBlock.getSwitchCases()) {
                if ((switchCase.getBasicBlock() != END) && (switchCase.getBasicBlock() != next)) {
                    writeLink(sb, id, switchCase.getBasicBlock(),
                            switchCase.isDefaultCase() ? "default" : "case: " + switchCase.getValue());
                }
            }
            break;
        case TYPE_TRY:
    ...
    public static String writePlantUMLUrl(String plantuml) throws Exception {
        ...
        if (deflater.finished()) {
            ...
            for (int i = 0; i < compressedDataLength; i += 3) {
                append3bytes(sb, output[i] & 0xFF, i + 1 < output.length ? output[i + 1] & 0xFF : 0,
