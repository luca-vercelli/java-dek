			...
			if (superConstructorCall != null) {
				...
				if (datas.size() == 1) {
					...
					if ((cfcd.getFlags() & FLAG_SYNTHETIC) != 0) {
						...
						if (superConstructorCall.getDescriptor().equals("()V")
								&& (firstLineNumber != UNKNOWN_LINE_NUMBER) && iterator.hasNext()) {
		...
		if (count > 0) {
			...
			for (Expression putField : putFields) {
				...
				if (declaration != null) {
					...
					((ClassFileFieldDeclaration) declaration.getFieldDeclaration())
