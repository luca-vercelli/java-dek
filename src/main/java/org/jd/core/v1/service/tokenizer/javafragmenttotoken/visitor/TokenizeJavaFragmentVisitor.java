...
public class TokenizeJavaFragmentVisitor implements JavaFragmentVisitor {
	...
	protected static final List<Token> EMPTY_WHILE_TOKENS = Arrays.asList(WHILE, TextToken.SPACE,
			TextToken.LEFTROUNDBRACKET, TRUE, TextToken.RIGHTROUNDBRACKET);
	...
	public void visit(ImportsFragment fragment) {
		...
		for (ImportsFragment.Import imp : imports) {
			...
			tokens.add(TextToken.SPACE);
			tokens.add(
					new ReferenceToken(ReferenceToken.TYPE, imp.getInternalName(), imp.getQualifiedName(), null, null));
	...
	public class KnownLineNumberTokenVisitor extends AbstractNopTokenVisitor {
		...
		public void visit(EndBlockToken token) {
			assert token != EndBlockToken.END_BLOCK
					: "Unexpected EndBlockToken.END_BLOCK at this step. Uses 'JavaFragmentFactory.addEnd***(fragments)' instead";
			tokens.add(token);
		}
		...
		public void visit(StartBlockToken token) {
			assert token != StartBlockToken.START_BLOCK
					: "Unexpected StartBlockToken.START_BLOCK at this step. Uses 'JavaFragmentFactory.addStart***(fragments)' instead";
			tokens.add(token);
		}
