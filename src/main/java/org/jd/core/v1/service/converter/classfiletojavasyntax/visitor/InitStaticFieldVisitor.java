...
public class InitStaticFieldVisitor extends AbstractJavaSyntaxVisitor {
	...
	protected boolean isAssertionsDisabledStatement(Statement statement) {
		...
		if (expression.getLeftExpression().isFieldReferenceExpression()) {
			...
			if ((fre.getType() == PrimitiveType.TYPE_BOOLEAN) && fre.getInternalTypeName().equals(internalTypeName)
					&& fre.getName().equals("$assertionsDisabled")) {
				return true;
			}
		}
				...
				if ((fdr != null) && (fdr.getVariableInitializer() == null)) {
					...
					if (((fdn.getFlags() & ACC_STATIC) != 0)
						...
						if (!searchLocalVariableReferenceVisitor.containsReference()) {
							...
							((ClassFileFieldDeclaration) fdr.getFieldDeclaration())
									.setFirstLineNumber(expression.getLineNumber());
							return true;
						}
					}
				}
	...
	protected void addStaticInitializerDeclaration(ClassFileStaticInitializerDeclaration sid, int lineNumber,
			BaseStatement statements) {
		methods.add(new ClassFileStaticInitializerDeclaration(sid.getBodyDeclaration(), sid.getClassFile(),
				sid.getMethod(), sid.getBindings(), sid.getTypeBounds(), lineNumber, statements));
	}
}
