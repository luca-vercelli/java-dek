...
public class Java5TypeParametersToTypeArgumentsBinder extends AbstractTypeParametersToTypeArgumentsBinder
    ...
    public Java5TypeParametersToTypeArgumentsBinder(TypeMaker typeMaker, String internalTypeName,
    ...
    @Override
    public ClassFileConstructorInvocationExpression newConstructorInvocationExpression(int lineNumber,
            ObjectType objectType, String descriptor, TypeMaker.MethodTypes methodTypes, BaseExpression parameters) {
        ...
        return new ClassFileConstructorInvocationExpression(lineNumber, objectType, descriptor, parameterTypes,
                parameters);
    }

    @Override
    public ClassFileSuperConstructorInvocationExpression newSuperConstructorInvocationExpression(int lineNumber,
            ObjectType objectType, String descriptor, TypeMaker.MethodTypes methodTypes, BaseExpression parameters) {
        ...
        if ((typeTypes != null) && (typeTypes.superType != null) && (typeTypes.superType.getTypeArguments() != null)) {
            ...
            if (superTypeTypes != null) {
                ...
                populateBindings(bindings, null, typeParameters, typeArguments, methodTypeParameters, TYPE_OBJECT, null,
                        null, null);
            }
        }
        ...
        return new ClassFileSuperConstructorInvocationExpression(lineNumber, objectType, descriptor, parameterTypes,
                parameters);
    }

    @Override
    public ClassFileMethodInvocationExpression newMethodInvocationExpression(int lineNumber, Expression expression,
            ObjectType objectType, String name, String descriptor, TypeMaker.MethodTypes methodTypes,
            BaseExpression parameters) {
        return new ClassFileMethodInvocationExpression(lineNumber, methodTypes.typeParameters, methodTypes.returnedType,
                expression, objectType.getInternalName(), name, descriptor, clone(methodTypes.parameterTypes),
                parameters);
    }

    @Override
    public FieldReferenceExpression newFieldReferenceExpression(int lineNumber, Type type, Expression expression,
            ObjectType objectType, String name, String descriptor) {
                ...
                if (type.isObjectType()) {
                    ...
                    if (ot.getTypeArguments() != null) {
                        ...
                        if (typeTypes == null) {
                            ...
                            boolean partialBinding = populateBindings(bindings, expression, typeParameters,
                                    typeArguments, null, TYPE_OBJECT, null, null, null);
        ...
        return new FieldReferenceExpression(lineNumber, type, expression, objectType.getInternalName(), name,
                descriptor);
    }
    ...
    protected boolean populateBindings(Map<String, TypeArgument> bindings, Expression expression,
        ...
        if (parameterTypes != null) {
            if (parameterTypes.isList() && parameters.isList()) {
                ...
                while (parametersIterator.hasNext()) {
                    populateBindingsWithTypeArgument(bindings, typeBounds, parameterTypesIterator.next(),
                            parametersIterator.next());
                }
            } else {
                populateBindingsWithTypeArgument(bindings, typeBounds, parameterTypes.getFirst(),
                        parameters.getFirst());
            }
        }
    ...
    protected boolean eraseTypeArguments(Expression expression, BaseTypeParameter typeParameters,
        ...
        if ((typeParameters != null) && (typeArguments == null) && (expression != null)) {
            ...
            if (expression.isFieldReferenceExpression() || expression.isMethodInvocationExpression()
                    || expression.isLocalVariableReferenceExpression()) {
                return true;
            }
        }
    ...
    protected void populateBindingsWithTypeArgument(Map<String, TypeArgument> bindings,
            Map<String, BaseType> typeBounds, Type type, Expression expression) {
    ...
    public void visit(MethodInvocationExpression expression) {
        ...
        if (!mie.isBound()) {
            ...
            if (staticMethod || (mie.getTypeParameters() != null)
                ...
                if (typeTypes != null) {
                    ...
                    if (exp.isSuperExpression()) {
                        ...
                        typeArguments = (typeTypes == null || typeTypes.superType == null) ? null
                                : typeTypes.superType.getTypeArguments();
                    ...
                    if ((methodTypeParameters != null) && !partialBinding) {
                        ...
                        mie.setNonWildcardTypeArguments(
                                bindTypeParametersToNonWildcardTypeArgumentsVisitor.getTypeArgument());
                    }
                    ...
                    if (expressionType.isObjectType()) {
                        ...
                        if (bindings.isEmpty() || partialBinding) {
                            ...
                            if (exp.isObjectTypeReferenceExpression() || (typeParameters == null)) {
                                ...
                                expressionType = expressionObjectType
                                        .createType(bindings.get(typeParameters.getFirst().getIdentifier()));
                            }
                        }
                    } else if (expressionType.isGenericType()) {
                        if (bindings.isEmpty() || partialBinding) {
                ...
                if (typeTypes != null) {
                    ...
                    if ((typeParameters != null) && (typeArguments == null)) {
                        if (typeParameters.isList()) {
                            ...
                            neObjectType = neObjectType
                                    .createType(new GenericType(typeParameters.getFirst().getIdentifier()));
                        }
                    }
    ...
    public void visit(CastExpression expression) {
        assert TYPE_OBJECT.equals(type) || (type.getDimension() == expression.getType().getDimension())
                : "TypeParametersToTypeArgumentsBinder.visit(CastExpression ce) : invalid array type";

        if (type.isObjectType()) {
            ObjectType objectType = (ObjectType) type;

            if ((objectType.getTypeArguments() != null)
                    && !objectType.getTypeArguments().equals(WildcardTypeArgument.WILDCARD_TYPE_ARGUMENT)) {
                assert expression.getType().isObjectType()
                        : "TypeParametersToTypeArgumentsBinder.visit(CastExpression ce) : invalid object type";
                ...
                if (objectType.getInternalName().equals(expressionObjectType.getInternalName())) {
                    ...
                    if (expressionExpressionType.isObjectType()) {
                        ...
                        } else if (objectType.getTypeArguments().isTypeArgumentAssignableFrom(contextualTypeBounds,
                                expressionExpressionObjectType.getTypeArguments())) {
                            expression.setType(objectType);
                        }
