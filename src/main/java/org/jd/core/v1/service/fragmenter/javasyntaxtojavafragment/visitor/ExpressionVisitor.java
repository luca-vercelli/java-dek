...
public class ExpressionVisitor extends TypeVisitor {
	...
	public void visit(ConstructorReferenceExpression expression) {
		...
		// tokens.add(new ReferenceToken(ReferenceToken.CONSTRUCTOR,
		// ot.getInternalName(), "new", expression.getDescriptor(),
	...
	public void visit(EnumConstantReferenceExpression expression) {
		...
		tokens.add(new ReferenceToken(ReferenceToken.FIELD, type.getInternalName(), expression.getName(),
				type.getDescriptor(), currentInternalTypeName));
	}
	...
	public void visit(FieldReferenceExpression expression) {
		if (expression.getExpression() == null) {
			...
			tokens.add(new ReferenceToken(ReferenceToken.FIELD, expression.getInternalTypeName(), expression.getName(),
	...
	public void visit(IntegerConstantExpression expression) {
		...
		case FLAG_CHAR:
			tokens.add(new CharacterConstantToken(CharacterUtil.escapeChar((char) expression.getIntegerValue()),
					currentInternalTypeName));
			break;
		case FLAG_BOOLEAN:
	...
	public void visit(MethodInvocationExpression expression) {
		...
		tokens.add(new ReferenceToken(ReferenceToken.METHOD, expression.getInternalTypeName(), expression.getName(),
				expression.getDescriptor(), currentInternalTypeName));
		tokens.add(StartBlockToken.START_PARAMETERS_BLOCK);
	...
	public void visit(StringConstantExpression expression) {
		tokens.addLineNumberToken(expression);
		tokens.add(
				new StringConstantToken(StringUtil.escapeString(expression.getStringValue()), currentInternalTypeName));
	}
	...
	public void visit(TernaryOperatorExpression expression) {
		...
		if (expression.getTrueExpression().isBooleanExpression()
				&& expression.getFalseExpression().isBooleanExpression()) {
	...
	protected void visit(Expression parent, Expression child) {
		if ((parent.getPriority() < child.getPriority())
				|| ((parent.getPriority() == 14) && (child.getPriority() == 13))) {
			tokens.add(TextToken.LEFTROUNDBRACKET);
			child.accept(this);
			tokens.add(TextToken.RIGHTROUNDBRACKET);
		} else {
	...
	protected class HexaExpressionVisitor implements org.jd.core.v1.model.javasyntax.expression.ExpressionVisitor {
		...
		public void visit(IntegerConstantExpression expression) {
			...
			case FLAG_BOOLEAN:
				...
				break;
			default:
				tokens.add(new NumericConstantToken(
						"0x" + Integer.toHexString(expression.getIntegerValue()).toUpperCase()));
				break;
			}
		}
		...
		public void visit(LongConstantExpression expression) {
			tokens.addLineNumberToken(expression);
			tokens.add(
					new NumericConstantToken("0x" + Long.toHexString(expression.getLongValue()).toUpperCase() + 'L'));
		}
