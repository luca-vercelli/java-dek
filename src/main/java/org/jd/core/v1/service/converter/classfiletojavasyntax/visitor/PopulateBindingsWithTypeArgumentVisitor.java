...
public class PopulateBindingsWithTypeArgumentVisitor implements TypeArgumentVisitor {
    ...
    public void init(Map<String, BaseType> contextualTypeBounds, Map<String, TypeArgument> bindings,
            Map<String, BaseType> typeBounds, BaseTypeArgument typeArgument) {
    ...
    public void visit(GenericType type) {
        ...
        if (bindings.containsKey(typeName)) {
            ...
            if (current != null) {
                if (current.isGenericTypeArgument() && !equals(contextualTypeBounds.get(typeName),
                        typeBounds.get(((GenericType) current).getName()))) {
                        ...
                        if (t1.isObjectType() && t2.isObjectType()) {
                            ...
                            if (!typeMaker.isAssignable(typeBounds, ot1, ot2)) {
                                if (typeMaker.isAssignable(typeBounds, ot2, ot1)) {
                                    bindings.put(typeName,
                                            checkTypeClassCheckDimensionAndReturnCurrentAsTypeArgument(type));
                                } else {
