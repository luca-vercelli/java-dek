...
public class TypeMaker {
    ...
    static {
        INTERNALNAME_TO_OBJECTPRIMITIVETYPE.put(ObjectType.TYPE_PRIMITIVE_BOOLEAN.getInternalName(),
                ObjectType.TYPE_PRIMITIVE_BOOLEAN);
        INTERNALNAME_TO_OBJECTPRIMITIVETYPE.put(ObjectType.TYPE_PRIMITIVE_BYTE.getInternalName(),
                ObjectType.TYPE_PRIMITIVE_BYTE);
        INTERNALNAME_TO_OBJECTPRIMITIVETYPE.put(ObjectType.TYPE_PRIMITIVE_CHAR.getInternalName(),
                ObjectType.TYPE_PRIMITIVE_CHAR);
        INTERNALNAME_TO_OBJECTPRIMITIVETYPE.put(ObjectType.TYPE_PRIMITIVE_DOUBLE.getInternalName(),
                ObjectType.TYPE_PRIMITIVE_DOUBLE);
        INTERNALNAME_TO_OBJECTPRIMITIVETYPE.put(ObjectType.TYPE_PRIMITIVE_FLOAT.getInternalName(),
                ObjectType.TYPE_PRIMITIVE_FLOAT);
        INTERNALNAME_TO_OBJECTPRIMITIVETYPE.put(ObjectType.TYPE_PRIMITIVE_INT.getInternalName(),
                ObjectType.TYPE_PRIMITIVE_INT);
        INTERNALNAME_TO_OBJECTPRIMITIVETYPE.put(ObjectType.TYPE_PRIMITIVE_LONG.getInternalName(),
                ObjectType.TYPE_PRIMITIVE_LONG);
        INTERNALNAME_TO_OBJECTPRIMITIVETYPE.put(ObjectType.TYPE_PRIMITIVE_SHORT.getInternalName(),
                ObjectType.TYPE_PRIMITIVE_SHORT);
        INTERNALNAME_TO_OBJECTPRIMITIVETYPE.put(ObjectType.TYPE_PRIMITIVE_VOID.getInternalName(),
                ObjectType.TYPE_PRIMITIVE_VOID);
    }

    private static final int INITIAL_SIZE = 1024;
    private Map<String, Type> signatureToType = new HashMap<>(INITIAL_SIZE);
    private Map<String, Type> internalTypeNameFieldNameToType = new HashMap<>(INITIAL_SIZE);
    private Map<String, ObjectType> descriptorToObjectType = new HashMap<>(INITIAL_SIZE);
    private Map<String, ObjectType> internalTypeNameToObjectType = new HashMap<>(INITIAL_SIZE);
    private Map<String, TypeTypes> internalTypeNameToTypeTypes = new HashMap<>(INITIAL_SIZE);
    private Map<String, Set<BaseType>> internalTypeNameMethodNameParameterCountToDeclaredParameterTypes = new HashMap<>(
            INITIAL_SIZE);
    private Map<String, Set<BaseType>> internalTypeNameMethodNameParameterCountToParameterTypes = new HashMap<>(
            INITIAL_SIZE);
    private Map<String, MethodTypes> internalTypeNameMethodNameDescriptorToMethodTypes = new HashMap<>(INITIAL_SIZE);
    private Map<String, MethodTypes> signatureToMethodTypes = new HashMap<>(INITIAL_SIZE);

    private Map<Long, Boolean> assignableRawTypes = new HashMap<>(INITIAL_SIZE);
    private Map<Long, ObjectType> superParameterizedObjectTypes = new HashMap<>(INITIAL_SIZE);

    private Map<String, String[]> hierarchy = new HashMap<>(INITIAL_SIZE);
    ...
    /**
     * Rules: ClassSignature: TypeParameters? SuperclassSignature
     * SuperInterfaceSignature* SuperclassSignature: ClassTypeSignature
    ...
    private MethodTypes parseMethodSignature(Method method, String key) {
        ...
        if (attributeSignature == null) {
            ...
            methodTypes = parseMethodSignature(method.getDescriptor(), attributeSignature.getSignature(),
                    exceptionTypeNames);
        }
    ...
    private MethodTypes parseMethodSignature(String descriptor, String signature, String[] exceptionTypeNames) {
        if (signature == null) {
            ...
            // Signature does not contain synthetic parameters like outer type name, for
            // example.
            ...
            if (mtDescriptor.parameterTypes == null) {
                ...
                UnmodifiableTypes parameterTypes = new UnmodifiableTypes(mtDescriptor.parameterTypes.getList()
                        .subList(0, mtDescriptor.parameterTypes.size() - mtSignature.parameterTypes.size()));
    ...
    /**
     * Rules: MethodTypeSignature: TypeParameters? '(' ReferenceTypeSignature* ')'
     * ReturnType ThrowsSignature* ReturnType: TypeSignature | VoidDescriptor
    ...
    /**
     * Rules: TypeParameter: Identifier ClassBound InterfaceBound* ClassBound: ':'
     * FieldTypeSignature? InterfaceBound: ':' FieldTypeSignature
     */
    ...
    /**
     * Rules: ClassTypeSignature: 'L' PackageSpecifier* SimpleClassTypeSignature
     * ClassTypeSignatureSuffix* ';' SimpleClassTypeSignature: Identifier
     * TypeArguments? ClassTypeSignatureSuffix: '.' SimpleClassTypeSignature
     */
    ...
    /**
     * Rules: ReferenceTypeSignature: ClassTypeSignature | ArrayTypeSignature |
     * TypeVariableSignature SimpleClassTypeSignature: Identifier TypeArguments?
     * ArrayTypeSignature: '[' TypeSignature TypeSignature: '[' FieldTypeSignature |
     * '[' BaseType BaseType: 'B' | 'C' | 'D' | 'F' | 'I' | 'J' | 'S' | 'Z'
    ...
    /**
     * Rules: TypeArgument: WildcardIndicator? FieldTypeSignature | '*'
     * WildcardIndicator: '+' | '-'
     */
    ...
    public ObjectType makeFromDescriptorOrInternalTypeName(String descriptorOrInternalTypeName) {
        return (descriptorOrInternalTypeName.charAt(0) == '[') ? makeFromDescriptor(descriptorOrInternalTypeName)
                : makeFromInternalTypeName(descriptorOrInternalTypeName);
    }
    ...
    public ObjectType makeFromDescriptor(String descriptor) {
        ...
        if (ot == null) {
            if (descriptor.charAt(0) == '[') {
                ...
                ot = (ObjectType) makeFromDescriptorWithoutBracket(descriptor.substring(dimension))
                        .createType(dimension);
            } else {
     ...
     * using <code>loadType()</code>; then, if not found, create the ObjectType
     * using create().
     * 
     */
    public ObjectType makeFromInternalTypeName(String internalTypeName) {
        assert (internalTypeName != null) && !internalTypeName.endsWith(";")
                : "ObjectTypeMaker.makeFromInternalTypeName(internalTypeName) : invalid internalTypeName";

        ObjectType ot = loadType(internalTypeName);

        if (ot == null) {
            // File not found with the system class loader -> Create type from
            // 'internalTypeName'
    ...
    /**
     * Create an ObjectType for given internalTypeName, without looking at that
     * class.
     */
    ...
    public ObjectType searchSuperParameterizedType(ObjectType superObjectType, ObjectType objectType) {
        if ((superObjectType == TYPE_UNDEFINED_OBJECT) || superObjectType.equals(TYPE_OBJECT)
                || superObjectType.equals(objectType)) {
            return objectType;
    ...
    /**
     * True if an object of type <code>right</code> is also of type
     * <code>left</code>
     */
    public boolean isAssignable(Map<String, BaseType> typeBounds, ObjectType left, ObjectType right) {
        if ((right == TYPE_UNDEFINED_OBJECT) || (left == TYPE_UNDEFINED_OBJECT) || left.equals(TYPE_OBJECT)
                || left.equals(right)) {
            return true;
        ...
        if (rightTypeTypes != null) {
            ...
            if ((rightTypeTypes.typeParameters == null) || (right.getTypeArguments() == null)) {
                ...
                if (rightTypeTypes.typeParameters.isList() && right.getTypeArguments().isTypeArgumentList()) {
                    ...
                    Iterator<TypeArgument> iteratorTypeArgument = right.getTypeArguments().getTypeArgumentList()
                            .iterator();
                    ...
                    bindings.put(rightTypeTypes.typeParameters.getFirst().getIdentifier(),
                            right.getTypeArguments().getTypeArgumentFirst());
                }
            }
                    ...
                    if (typeTypes != null) {
                        ...
                        if ((type == null) && (typeTypes.interfaces != null)) {
                            if (typeTypes.interfaces.isList()) {
                                ...
                                type = loadFieldType((ObjectType) typeTypes.interfaces.getFirst(), fieldName,
                                        descriptor);
                            }
                        }
                    }
        ...
        if ((type != null) && (typeArguments != null)) {
            ...
            if ((typeTypes != null) && (typeTypes.typeParameters != null)) {
                ...
                if (typeTypes.typeParameters.isList() && typeArguments.isTypeArgumentList()) {
                    ...
                    bindings.put(typeTypes.typeParameters.getFirst().getIdentifier(),
                    ...
                    if (typeTypes != null) {
                        ...
                        if ((methodTypes == null) && (typeTypes.interfaces != null)) {
                            if (typeTypes.interfaces.isList()) {
                                ...
                                methodTypes = loadMethodTypes((ObjectType) typeTypes.interfaces.getFirst(), methodName,
                                        descriptor);
                            }
                        }
                    }
    ...
    /**
     * Load objectType with given internalTypeName using either given Loader or a
     * ClassPathLoader. objectType is internally cached in
    ...
    /**
     * Create an ObjectType from a Class.
     * 
     * @param data byte array content of class
     */
        ...
        if (data != null) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (parameterCount > 0) {
                    if (set == null) {
                        set = new HashSet<>();
                        internalTypeNameMethodNameParameterCountToDeclaredParameterTypes.put(key, set);
                    }
        ...
        if (setOfParameterTypes == null) {
            ...
            if (!constructor) {
                ...
                if ((typeTypes != null) && (typeTypes.superType != null)) {
                    setOfParameterTypes.addAll(
                            getSetOfParameterTypes(typeTypes.superType.getInternalName(), suffixKey, constructor));
                }
            }

            Set<BaseType> declaredParameterTypes = internalTypeNameMethodNameParameterCountToDeclaredParameterTypes
                    .get(key);
