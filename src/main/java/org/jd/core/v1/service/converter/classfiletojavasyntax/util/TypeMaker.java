...
public class TypeMaker {
	...
	public MethodTypes makeMethodTypes(String internalTypeName, String methodName, String descriptor) {
		...
		if (methodTypes == null) {
			...
			internalTypeNameMethodNameDescriptorToMethodTypes.put(key,
	...
	private ObjectType loadType(String internalTypeName) {
		...
		if (ot == null) {
			try {
				if (loader.canLoad(internalTypeName)) {
					internalTypeNameToObjectType.put(internalTypeName,
	...
	private void loadFieldsAndMethods(String internalTypeName, byte[] data) throws Exception {
		if (data != null) {
			...
			for (int i = 0; i < count; i++) {
				...
				if (parameterCount > 0) {
					if (set == null) {
						internalTypeNameMethodNameParameterCountToDeclaredParameterTypes.put(key,
								set = new HashSet<>());
					}
