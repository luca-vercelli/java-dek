...
public class ByteCodeParser {
    ...
    public ByteCodeParser(TypeMaker typeMaker, LocalVariableMaker localVariableMaker, ClassFile classFile,
        ...
        if (this.genericTypesSupported) {
            this.typeParametersToTypeArgumentsBinder = new Java5TypeParametersToTypeArgumentsBinder(typeMaker,
                    this.internalTypeName, comd);
        } else {
        ...
        for (int offset = fromOffset; offset < toOffset; offset++) {
            ...
            case 185: // INVOKEVIRTUAL, INVOKESPECIAL, INVOKESTATIC, INVOKEINTERFACE
                ...
                if (opcode == 184) { // INVOKESTATIC
                    ...
                    if (TYPE_VOID.equals(methodTypes.returnedType)) {
                        typeParametersToTypeArgumentsBinder.bindParameterTypesWithArgumentTypes(TYPE_OBJECT,
                                expression1);
                        statements.add(new ExpressionStatement(expression1));
                    } else {
                        stack.push(expression1);
                    ...
                    if (TYPE_VOID.equals(methodTypes.returnedType)) {
                        if ((opcode == 183) && // INVOKESPECIAL
                            ...
                            if (expression1.isNewExpression()) {
                                typeParametersToTypeArgumentsBinder.updateNewExpression(
                                        (ClassFileNewExpression) expression1, descriptor, methodTypes, parameters);
                            } else if (ot.getDescriptor().equals(expression1.getType().getDescriptor())) {
                                statements.add(new ExpressionStatement(
                                        typeParametersToTypeArgumentsBinder.newConstructorInvocationExpression(
                                                lineNumber, ot, descriptor, methodTypes, parameters)));
                            } else {
                                statements.add(new ExpressionStatement(
                                        typeParametersToTypeArgumentsBinder.newSuperConstructorInvocationExpression(
                                                lineNumber, ot, descriptor, methodTypes, parameters)));
                            }
                        } else {
                            expression1 = typeParametersToTypeArgumentsBinder.newMethodInvocationExpression(lineNumber,
                                    getMethodInstanceReference(expression1, ot, name, descriptor), ot, name, descriptor,
                        ...
                        stack.push(typeParametersToTypeArgumentsBinder.newMethodInvocationExpression(lineNumber,
                                getMethodInstanceReference(expression1, ot, name, descriptor), ot, name, descriptor,
                                methodTypes, parameters));
                    }
                }
            ...
            case 192: // CHECKCAST
                ...
                if (type1.isObjectType() && expression1.getType().isObjectType()
                        && typeMaker.isRawTypeAssignable((ObjectType) type1, (ObjectType) expression1.getType())) {
                    // Ignore cast
                    ...
                    stack.push(new CastExpression(searchFirstLineNumberVisitor.getLineNumber(), type1,
    ...
    private BaseExpression extractParametersFromStack(Statements statements, DefaultStack<Expression> stack,
    ...
    private AbstractLocalVariable getLocalVariableInAssignment(int index, int offset, Expression value) {
        ...
        if (value.isNullExpression()) {
            ...
            AbstractLocalVariable valueLocalVariable = ((ClassFileLocalVariableReferenceExpression) value)
                    .getLocalVariable();
            AbstractLocalVariable lv = localVariableMaker.getLocalVariableInAssignment(typeBounds, index, offset,
                    valueLocalVariable);
    ...
    private void parseLDC(DefaultStack<Expression> stack, ConstantPool constants, int lineNumber, Constant constant) {
        ...
        case CONSTANT_Float:
            ...
            if (f == Float.MIN_VALUE) {
                stack.push(new FieldReferenceExpression(lineNumber, TYPE_FLOAT,
                        new ObjectTypeReferenceExpression(lineNumber, ObjectType.TYPE_FLOAT), "java/lang/Float",
                        "MIN_VALUE", "F"));
                ...
                stack.push(new FieldReferenceExpression(lineNumber, TYPE_FLOAT,
                        new ObjectTypeReferenceExpression(lineNumber, ObjectType.TYPE_FLOAT), "java/lang/Float", "NaN",
                        "F"));
            } else {
        ...
        case CONSTANT_Long:
            ...
            if (l == Long.MIN_VALUE) {
                stack.push(new FieldReferenceExpression(lineNumber, TYPE_LONG,
                        new ObjectTypeReferenceExpression(lineNumber, ObjectType.TYPE_LONG), "java/lang/Long",
                        "MIN_VALUE", "J"));
        ...
        case CONSTANT_Double:
            ...
            if (d == Double.MIN_VALUE) {
                stack.push(new FieldReferenceExpression(lineNumber, TYPE_DOUBLE,
                        new ObjectTypeReferenceExpression(lineNumber, ObjectType.TYPE_DOUBLE), "java/lang/Double",
                        "MIN_VALUE", "D"));
                ...
                stack.push(new FieldReferenceExpression(lineNumber, TYPE_DOUBLE,
                        new ObjectTypeReferenceExpression(lineNumber, ObjectType.TYPE_MATH), "java/lang/Math", "E",
                        "D"));
            } else if (d == Math.PI) {
                stack.push(new FieldReferenceExpression(lineNumber, TYPE_DOUBLE,
                        new ObjectTypeReferenceExpression(lineNumber, ObjectType.TYPE_MATH), "java/lang/Math", "PI",
                        "D"));
            } else {
    ...
    private static void parseILOAD(Statements statements, DefaultStack<Expression> stack, int lineNumber, int offset,
            AbstractLocalVariable localVariable) {
        if (!statements.isEmpty()) {
            Expression expression = statements.getLast().getExpression();

            if ((expression.getLineNumber() == lineNumber) && expression.isPreOperatorExpression()
    ...
    private void parseSTORE(Statements statements, DefaultStack<Expression> stack, int lineNumber, int offset,
            AbstractLocalVariable localVariable, Expression valueRef) {
        ClassFileLocalVariableReferenceExpression vre = new ClassFileLocalVariableReferenceExpression(lineNumber,
                offset, localVariable);

        typeParametersToTypeArgumentsBinder.bindParameterTypesWithArgumentTypes(vre.getType(), valueRef);

        if ((valueRef.getLineNumber() == lineNumber) && valueRef.isBinaryOperatorExpression()
                && valueRef.getLeftExpression().isLocalVariableReferenceExpression()) {
            ClassFileLocalVariableReferenceExpression lvr = (ClassFileLocalVariableReferenceExpression) valueRef
                    .getLeftExpression();
                ...
                case "+":
                    if (isPositiveOne(boe.getRightExpression())) {
                        if (stackContainsLocalVariableReference(stack, localVariable)) {
                            stack.pop();
                            stack.push(valueRef);
                            expression = newPostArithmeticOperatorExpression(boe.getLineNumber(),
                                    boe.getLeftExpression(), "++");
                        } else {
                            expression = newPreArithmeticOperatorExpression(boe.getLineNumber(), "++",
                                    boe.getLeftExpression());
                        }
    ...
    private static boolean stackContainsLocalVariableReference(DefaultStack<Expression> stack,
            AbstractLocalVariable localVariable) {
        if (!stack.isEmpty()) {
    ...
    private void parsePUT(Statements statements, DefaultStack<Expression> stack, int lineNumber,
            FieldReferenceExpression fr, Expression valueRef) {
    ...
    private void parseInvokeDynamic(Statements statements, DefaultStack<Expression> stack, ConstantPool constants,
        ...
        if (!statements.isEmpty()) {
            ...
            if (expression.isMethodInvocationExpression()) {
                ...
                if (mie.getName().equals("getClass") && mie.getDescriptor().equals("()Ljava/lang/Class;")
                        && mie.getInternalTypeName().equals("java/lang/Object")) {
                    statements.removeLast();
                }
            }
        }
        ...
        if (typeName.equals(internalTypeName)) {
            for (ClassFileConstructorOrMethodDeclaration methodDeclaration : bodyDeclaration.getMethodDeclarations()) {
                if (((methodDeclaration.getFlags() & (ACC_SYNTHETIC | ACC_PRIVATE)) == (ACC_SYNTHETIC | ACC_PRIVATE))
                    ...
                    stack.push(new LambdaIdentifiersExpression(lineNumber, indyMethodTypes.returnedType,
                            indyMethodTypes.returnedType,
        ...
        if (indyParameters == null) {
            ...
            if (name1.equals("<init>")) {
                stack.push(
                        new ConstructorReferenceExpression(lineNumber, indyMethodTypes.returnedType, ot, descriptor1));
            } else {
                stack.push(new MethodReferenceExpression(lineNumber, indyMethodTypes.returnedType,
                        new ObjectTypeReferenceExpression(lineNumber, ot), typeName, name1, descriptor1));
            }
            return;
        }
    ...
    private static boolean stackContainsFieldReference(DefaultStack<Expression> stack, FieldReferenceExpression fr) {
        if (!stack.isEmpty()) {
            ...
            if (expression.isFieldReferenceExpression()) {
                return expression.getName().equals(fr.getName())
                        && expression.getExpression().getType().equals(fr.getExpression().getType());
            }
        }
    ...
    private void parseASTORE(Statements statements, DefaultStack<Expression> stack, int lineNumber, int offset,
            AbstractLocalVariable localVariable, Expression valueRef) {
    ...
    private void createAssignment(Statements statements, DefaultStack<Expression> stack, int lineNumber,
        ...
        if (!statements.isEmpty()) {
            ...
            if (lastStatement.isExpressionStatement()) {
                ...
                if (lastExpression.isBinaryOperatorExpression()) {
                    ...
                    if ((lineNumber > 0) && (lastExpression.getLineNumber() == lineNumber)
                            && (lastExpression.getLeftExpression().getClass() == rightExpression.getClass())) {
                        if (rightExpression.isLocalVariableReferenceExpression()) {
                            ClassFileLocalVariableReferenceExpression lvr1 = (ClassFileLocalVariableReferenceExpression) lastExpression
                                    .getLeftExpression();
                            ...
                            FieldReferenceExpression fr1 = (FieldReferenceExpression) lastExpression
                                    .getLeftExpression();
                    ...
                    if (lastExpression.getExpression().getClass() == rightExpression.getClass()) {
                        if (rightExpression.isLocalVariableReferenceExpression()) {
                            ...
                            if (lvr1.getLocalVariable() == lvr2.getLocalVariable()) {
                                rightExpression = newPreArithmeticOperatorExpression(lastExpression.getLineNumber(),
                                        lastExpression.getOperator(), lastExpression.getExpression());
                                statements.removeLast();
                            }
                        } else if (rightExpression.isFieldReferenceExpression()) {
    ...
    private void parseIINC(Statements statements, DefaultStack<Expression> stack, int lineNumber, int offset,
            AbstractLocalVariable localVariable, int count) {
        Expression expression;
    ...
    private void parseIF(DefaultStack<Expression> stack, int lineNumber, BasicBlock basicBlock, String operator1,
            String operator2, int priority) {
        Expression expression = stack.pop();

        if (expression.getClass() == ClassFileCmpExpression.class) {
            ClassFileCmpExpression cmp = (ClassFileCmpExpression) expression;

            typeParametersToTypeArgumentsBinder.bindParameterTypesWithArgumentTypes(cmp.getLeftExpression().getType(),
                    cmp.getLeftExpression());
            typeParametersToTypeArgumentsBinder.bindParameterTypesWithArgumentTypes(cmp.getRightExpression().getType(),
                    cmp.getRightExpression());

            stack.push(new BinaryOperatorExpression(lineNumber, TYPE_BOOLEAN, cmp.getLeftExpression(),
                    (basicBlock.mustInverseCondition() ? operator1 : operator2), cmp.getRightExpression(), priority));
    ...
    private void parseGetStatic(DefaultStack<Expression> stack, ConstantPool constants, int lineNumber, int index) {
        ...
        Expression objectRef = new ObjectTypeReferenceExpression(lineNumber, ot,
                !internalTypeName.equals(typeName) || localVariableMaker.containsName(name));
        stack.push(typeParametersToTypeArgumentsBinder.newFieldReferenceExpression(lineNumber, type, objectRef, ot,
                name, descriptor));
    }

    private void parsePutStatic(Statements statements, DefaultStack<Expression> stack, ConstantPool constants,
            int lineNumber, int index) {
        ConstantMemberRef constantMemberRef = constants.getConstant(index);
        String typeName = constants.getConstantTypeName(constantMemberRef.getClassIndex());
        ObjectType ot = typeMaker.makeFromInternalTypeName(typeName);
        ...
        Expression objectRef = new ObjectTypeReferenceExpression(lineNumber, ot,
                !internalTypeName.equals(typeName) || localVariableMaker.containsName(name));
        FieldReferenceExpression fieldRef = typeParametersToTypeArgumentsBinder.newFieldReferenceExpression(lineNumber,
                type, objectRef, ot, name, descriptor);
    ...
    private void parseGetField(DefaultStack<Expression> stack, ConstantPool constants, int lineNumber, int index) {
        ...
        stack.push(typeParametersToTypeArgumentsBinder.newFieldReferenceExpression(lineNumber, type,
                getFieldInstanceReference(objectRef, ot, name), ot, name, descriptor));
    }

    private void parsePutField(Statements statements, DefaultStack<Expression> stack, ConstantPool constants,
            int lineNumber, int index) {
        ConstantMemberRef constantMemberRef = constants.getConstant(index);
        String typeName = constants.getConstantTypeName(constantMemberRef.getClassIndex());
        ObjectType ot = typeMaker.makeFromInternalTypeName(typeName);
        ...
        Expression objectRef = stack.pop();
        FieldReferenceExpression fieldRef = typeParametersToTypeArgumentsBinder.newFieldReferenceExpression(lineNumber,
                type, getFieldInstanceReference(objectRef, ot, name), ot, name, descriptor);
        parsePUT(statements, stack, lineNumber, fieldRef, valueRef);
    }
        ...
        if ((objectType.getQualifiedName() == null) && (objectType.getName() == null)) {
            ...
            if (typeDeclaration == null) {
                ...
                if (declaration.getInterfaces() != null) {
                    return new ClassFileNewExpression(lineNumber, (ObjectType) declaration.getInterfaces(),
     ...
     * https://docs.oracle.com/javase/specs/jls/se7/html/jls-15.html#jls-15.18.2 See
     * "Shift Operators":
     * https://docs.oracle.com/javase/specs/jls/se7/html/jls-15.html#jls-15.19
     */
    private Expression newIntegerBinaryOperatorExpression(int lineNumber, Expression leftExpression, String operator,
            Expression rightExpression, int priority) {
        if (leftExpression.isLocalVariableReferenceExpression()) {
            AbstractLocalVariable leftVariable = ((ClassFileLocalVariableReferenceExpression) leftExpression)
                    .getLocalVariable();
    ...
    /*
     * Operators = { "&", "|", "^" } See "Binary Numeric Promotion":
     * https://docs.oracle.com/javase/specs/jls/se7/html/jls-15.html#jls-15.22.1
     */
    private Expression newIntegerOrBooleanBinaryOperatorExpression(int lineNumber, Expression leftExpression,
            String operator, Expression rightExpression, int priority) {
        Type type = TYPE_INT;
        ...
        if (leftExpression.isLocalVariableReferenceExpression()) {
            ...
            if (rightExpression.isLocalVariableReferenceExpression()) {
                ...
                if (leftVariable.isAssignableFrom(typeBounds, TYPE_BOOLEAN)
                        || rightVariable.isAssignableFrom(typeBounds, TYPE_BOOLEAN)) {
        ...
        typeParametersToTypeArgumentsBinder.bindParameterTypesWithArgumentTypes(leftExpression.getType(),
                rightExpression);
        typeParametersToTypeArgumentsBinder.bindParameterTypesWithArgumentTypes(rightExpression.getType(),
                leftExpression);
    ...
    /*
     * Operators = { "==", "!=" } See "Numerical Equality Operators == and !=":
     * https://docs.oracle.com/javase/specs/jls/se7/html/jls-15.html#jls-15.21.1
     */
    private Expression newIntegerOrBooleanComparisonOperatorExpression(int lineNumber, Expression leftExpression,
            String operator, Expression rightExpression, int priority) {
        if (leftExpression.isLocalVariableReferenceExpression()) {
            AbstractLocalVariable leftVariable = ((ClassFileLocalVariableReferenceExpression) leftExpression)
                    .getLocalVariable();
        ...
        return new BinaryOperatorExpression(lineNumber, TYPE_BOOLEAN, leftExpression, operator, rightExpression,
                priority);
    }
     ...
     * https://docs.oracle.com/javase/specs/jls/se7/html/jls-15.html#jls-15.21.1
     */
    private Expression newIntegerComparisonOperatorExpression(int lineNumber, Expression leftExpression,
            String operator, Expression rightExpression, int priority) {
        if (leftExpression.isLocalVariableReferenceExpression()) {
            AbstractLocalVariable leftVariable = ((ClassFileLocalVariableReferenceExpression) leftExpression)
                    .getLocalVariable();
     ...
     * @return expression, 'this' or 'super'
     */
    private Expression getMethodInstanceReference(Expression expression, ObjectType ot, String name,
            String descriptor) {
    ...
    protected static Expression forceExplicitCastExpression(Expression expression) {
        // In case of downcasting, set all cast expression children as explicit to
        // prevent missing castings
