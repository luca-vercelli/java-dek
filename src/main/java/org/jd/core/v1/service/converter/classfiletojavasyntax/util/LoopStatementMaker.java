...
public class LoopStatementMaker {
	...
	public static Statement makeLoop(int majorVersion, Map<String, BaseType> typeBounds,
			LocalVariableMaker localVariableMaker, BasicBlock loopBasicBlock, Statements statements,
			Expression condition, Statements subStatements, Statements jumps) {
		Statement loop = makeLoop(majorVersion, typeBounds, localVariableMaker, loopBasicBlock, statements, condition,
				subStatements);
	...
	protected static Statement makeLoop(int majorVersion, Map<String, BaseType> typeBounds,
		...
		if (forEachSupported) {
			Statement statement = makeForEachArray(typeBounds, localVariableMaker, statements, condition,
					subStatements);
		...
		case 0:
			if (lineNumber > 0) {
				...
				if (init != null) {
					return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(),
							loopBasicBlock.getToOffset(), init, condition, null, null);
				}
			}
			break;
		case 1:
			if (lineNumber > 0) {
				...
				if (subStatement.isExpressionStatement()) {
					...
					if (subExpression.getLineNumber() == lineNumber) {
						return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(),
								loopBasicBlock.getToOffset(), init, condition, subExpression, null);
					} else if (init != null) {
						return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(),
								loopBasicBlock.getToOffset(), init, condition, null, subStatement);
					}
				} else if (init != null) {
					return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(),
							loopBasicBlock.getToOffset(), init, condition, null, subStatement);
				...
				return createForStatementWithoutLineNumber(localVariableMaker, loopBasicBlock, statements, condition,
						subStatements);
			}
			break;
		default:
			if (lineNumber > 0) {
				...
				if ((init != null) || (update.size() > 0)) {
					return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(),
							loopBasicBlock.getToOffset(), init, condition, update, subStatements);
				}
			} else {
				// Unknown line numbers => Just try to find 'for (expression;;expression)'
				return createForStatementWithoutLineNumber(localVariableMaker, loopBasicBlock, statements, condition,
	...
	public static Statement makeLoop(LocalVariableMaker localVariableMaker, BasicBlock loopBasicBlock,
			Statements statements, Statements subStatements, Statements jumps) {
		subStatements.accept(REMOVE_LAST_CONTINUE_STATEMENT_VISITOR);
	...
	protected static Statement makeLoop(LocalVariableMaker localVariableMaker, BasicBlock loopBasicBlock,
			Statements statements, Statements subStatements) {
		int subStatementsSize = subStatements.size();
		...
		case 1:
			...
			if (subStatement.isExpressionStatement()) {
				...
				if (lineNumber > 0) {
					...
					if (init != null) {
						return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(),
								loopBasicBlock.getToOffset(), init, null, subExpression, null);
					}
				}
			}
			break;
		default:
			...
			if (firstLineNumber > 0) {
				...
				if (update.size() > 0) {
					...
					return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(),
							loopBasicBlock.getToOffset(), init, null, update, subStatements);
				}
			} else {
				// Unknown line numbers => Just try to find 'for (expression;;expression)'
				Statement statement = createForStatementWithoutLineNumber(localVariableMaker, loopBasicBlock,
						statements, BooleanExpression.TRUE, subStatements);
	...
	public static Statement makeDoWhileLoop(BasicBlock loopBasicBlock, Expression condition, Statements subStatements,
			Statements jumps) {
		subStatements.accept(REMOVE_LAST_CONTINUE_STATEMENT_VISITOR);
	...
	protected static Statement createForStatementWithoutLineNumber(LocalVariableMaker localVariableMaker,
			BasicBlock basicBlock, Statements statements, Expression condition, Statements subStatements) {

		if (!statements.isEmpty()) {
			Expression init = statements.getLast().getExpression();

			if (init.getLeftExpression().isLocalVariableReferenceExpression()) {
				AbstractLocalVariable localVariable = ((ClassFileLocalVariableReferenceExpression) init
				...
				if (expression.isLocalVariableReferenceExpression()
						&& (((ClassFileLocalVariableReferenceExpression) expression)
					...
					return newClassFileForStatement(localVariableMaker, basicBlock.getFromOffset(),
							basicBlock.getToOffset(), init, condition, update, subStatements);
				}
			}
		}
	...
	protected static Statement makeForEachArray(Map<String, BaseType> typeBounds, LocalVariableMaker localVariableMaker,
		...
		if (!expression.getRightExpression().isLengthExpression()
		...
		AbstractLocalVariable syntheticArray = ((ClassFileLocalVariableReferenceExpression) expression)
				.getLocalVariable();
		AbstractLocalVariable syntheticLength = ((ClassFileLocalVariableReferenceExpression) boe.getLeftExpression())
				.getLocalVariable();
		...
		if (!arrayExpression.getExpression().isLocalVariableReferenceExpression()
				|| !arrayExpression.getIndex().isLocalVariableReferenceExpression()) {
			return null;
		}
		if (((ClassFileLocalVariableReferenceExpression) arrayExpression.getExpression())
				.getLocalVariable() != syntheticArray) {
			return null;
		}

		AbstractLocalVariable syntheticIndex = ((ClassFileLocalVariableReferenceExpression) arrayExpression.getIndex())
				.getLocalVariable();
		AbstractLocalVariable item = ((ClassFileLocalVariableReferenceExpression) expression.getLeftExpression())
				.getLocalVariable();
		...
		if (((ClassFileLocalVariableReferenceExpression) expression.getLeftExpression())
				.getLocalVariable() != syntheticArray) {
			return null;
		}
		...
		if ((expression.getRightExpression().getIntegerValue() != 0)
				|| (((ClassFileLocalVariableReferenceExpression) expression.getLeftExpression())
						.getLocalVariable() != syntheticIndex)) {
			return null;
		}

		// i$ < len$;
		if (!condition.getLeftExpression().isLocalVariableReferenceExpression()
				|| !condition.getRightExpression().isLocalVariableReferenceExpression()) {
			return null;
		}
		if ((((ClassFileLocalVariableReferenceExpression) condition.getLeftExpression())
				.getLocalVariable() != syntheticIndex)
				|| (((ClassFileLocalVariableReferenceExpression) condition.getRightExpression())
						.getLocalVariable() != syntheticLength)) {
			return null;
		}
	...
	protected static Statement makeForEachList(Map<String, BaseType> typeBounds, LocalVariableMaker localVariableMaker,
			Statements statements, Expression condition, Statements subStatements) {
		if (condition == null) {
			return null;
		}
		...
		AbstractLocalVariable syntheticIterator = ((ClassFileLocalVariableReferenceExpression) mie.getExpression())
				.getLocalVariable();

		// Iterator i$ = list.iterator();
		Expression boe = statements.getLast().getExpression();

		if ((boe == null) || !boe.getLeftExpression().isLocalVariableReferenceExpression()
		...
		if (((ClassFileLocalVariableReferenceExpression) boe.getLeftExpression())
				.getLocalVariable() != syntheticIterator) {
			return null;
		}
		...
		if (!mie.getName().equals("next") || !mie.getInternalTypeName().equals("java/util/Iterator")
				|| !mie.getExpression().isLocalVariableReferenceExpression()) {
			return null;
		}
		...
		// Found
		AbstractLocalVariable item = ((ClassFileLocalVariableReferenceExpression) boe.getLeftExpression())
				.getLocalVariable();
	...
	protected static Statement makeLabels(int loopIndex, int continueOffset, int breakOffset, Statement loop,
			Statements jumps) {
		if (!jumps.isEmpty()) {
	...
	protected static ClassFileForStatement newClassFileForStatement(LocalVariableMaker localVariableMaker,
			int fromOffset, int toOffset, BaseExpression init, Expression condition, BaseExpression update,
