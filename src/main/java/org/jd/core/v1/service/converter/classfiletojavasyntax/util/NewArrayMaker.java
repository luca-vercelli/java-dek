...
public class NewArrayMaker {
    ...
    public static Expression make(Statements statements, Expression newArray) {
        if (!statements.isEmpty()) {
            ...
            if ((ae.getExpression() == newArray) && ae.getIndex().isIntegerConstantExpression()) {
                return new NewInitializedArray(newArray.getLineNumber(), newArray.getType(),
                        createVariableInitializer(statements.listIterator(statements.size()), newArray));
            }
        }
    ...
    protected static ArrayVariableInitializer createVariableInitializer(ListIterator<Statement> li,
            Expression newArray) {
        ...
        while (vii.hasNext()) {
            ...
            if (expression.isNewArray()) {
                ...
                if (del.isIntegerConstantExpression() && (del.getIntegerValue() == 0)) {
                    ...
                    if ((type.getDimension() == t.getDimension() + 1)
                            && (type.getDescriptor().length() == t.getDescriptor().length() + 1)
                            && type.getDescriptor().endsWith(t.getDescriptor())) {
                        vii.set(EMPTY_ARRAY);
                    }
                }
            }
