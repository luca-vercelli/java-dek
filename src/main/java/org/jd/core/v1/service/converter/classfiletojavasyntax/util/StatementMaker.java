...
public class StatementMaker {
	...
	protected static final NullExpression FINALLY_EXCEPTION_EXPRESSION = new NullExpression(
			new ObjectType("java/lang/Exception", "java.lang.Exception", "Exception"));
	...
	public Statements make(ControlFlowGraph cfg) {
		...
		// Replace pattern "synthetic_local_var = ...; return synthetic_local_var;" with
		// "return ...;"
	...
	protected void makeStatements(WatchDog watchdog, BasicBlock basicBlock, Statements statements, Statements jumps) {
		...
		case TYPE_IF_ELSE:
			...
			if (!basicBlock.getSub2().matchType(TYPE_LOOP_END | TYPE_LOOP_CONTINUE | TYPE_LOOP_START)
					&& (stack.size() != backup.size())) {
				stack.copy(backup);
			}
		...
		case TYPE_CONDITION_OR:
			...
			stack.push(new BinaryOperatorExpression(basicBlock.getFirstLineNumber(), PrimitiveType.TYPE_BOOLEAN, exp1,
					"||", exp2, 14));
			break;
	...
	protected Statements makeSubStatements(WatchDog watchdog, BasicBlock basicBlock, Statements statements,
			Statements jumps, BasicBlock updateBasicBlock) {
	...
	protected Expression makeExpression(WatchDog watchdog, BasicBlock basicBlock, Statements statements,
			Statements jumps) {
	...
	protected void parseSwitch(WatchDog watchdog, BasicBlock basicBlock, Statements statements, Statements jumps) {
		...
		for (int i = 0, len = switchCases.size(); i < len; i++) {
			...
			if (sc.isDefaultCase()) {
				...
				SwitchStatement.Label label = new SwitchStatement.ExpressionLabel(
						new IntegerConstantExpression(conditionType, sc.getValue()));
				...
				for (; i < j; i++) {
					labels.add(new SwitchStatement.ExpressionLabel(
							new IntegerConstantExpression(conditionType, switchCases.get(i).getValue())));
				}
	...
	protected void parseTry(WatchDog watchdog, BasicBlock basicBlock, Statements statements, Statements jumps,
		...
		for (ExceptionHandler exceptionHandler : basicBlock.getExceptionHandlers()) {
			...
			if (exceptionHandler.getInternalThrowableName() == null) {
				...
				stack.push(new NullExpression(
						typeMaker.makeFromInternalTypeName(exceptionHandler.getInternalThrowableName())));
				...
				ClassFileTryStatement.CatchClause cc = new ClassFileTryStatement.CatchClause(lineNumber, ot, exception,
						catchStatements);
		...
		if ((finallyStatements != null) && (finallyStatements.size() > 0)
	...
	protected void removeExceptionReference(Statements catchStatements) {
		if ((catchStatements.size() > 0) && catchStatements.getFirst().isExpressionStatement()) {
			...
			if (exp.isBinaryOperatorExpression()) {
				if (exp.getLeftExpression().isLocalVariableReferenceExpression()
						&& exp.getRightExpression().isNullExpression()) {
	...
	protected void parseJSR(WatchDog watchdog, BasicBlock basicBlock, Statements statements, Statements jumps) {
		...
		ClassFileLocalVariableReferenceExpression vre = (ClassFileLocalVariableReferenceExpression) boe
				.getLeftExpression();
	...
	protected void parseIf(WatchDog watchdog, BasicBlock basicBlock, Statements statements, Statements jumps) {
		...
		if (ByteCodeParser.isAssertCondition(internalTypeName, condition)) {
			...
			if ((subStatements.size() == 1) && (index + 1 == statements.size())
				...
				if (cond.getLineNumber() >= cfres1.getLineNumber()) {
					...
					if (cfres1.getLineNumber() == cfres2.getLineNumber()) {
						...
						statements
								.add(new ReturnExpressionStatement(newTernaryOperatorExpression(cfres1.getLineNumber(),
										cond, cfres1.getExpression(), cfres2.getExpression())));
					}
				}
			}
		}
	...
	protected void parseLoop(WatchDog watchdog, BasicBlock basicBlock, Statements statements, Statements jumps) {
		...
		if (sub1.getType() == TYPE_IF) {
			...
			if (ifBB.getNext() == LOOP_END) {
				...
				statements.add(LoopStatementMaker.makeLoop(majorVersion, typeBounds, localVariableMaker, basicBlock,
		...
		if ((next == LOOP_START) && (last.getType() == TYPE_IF) && (last.getSub1() == LOOP_END)
				&& (countStartLoop(sub1) == 1)) {
			// 'do-while'
			...
			// Infinite loop
			statements.add(LoopStatementMaker.makeLoop(localVariableMaker, basicBlock, statements,
					makeSubStatements(watchdog, sub1, statements, jumps, updateBasicBlock), jumps));
		}
	...
	protected Expression parseTernaryOperator(int lineNumber, Expression condition, Expression exp1, Expression exp2) {
		if (ObjectType.TYPE_CLASS.equals(exp1.getType()) && ObjectType.TYPE_CLASS.equals(exp2.getType())
				&& condition.isBinaryOperatorExpression()) {

			if (condition.getLeftExpression().isFieldReferenceExpression()
					&& condition.getRightExpression().isNullExpression()) {
				...
				if (freCond.getInternalTypeName().equals(internalTypeName)) {
					...
					if (fieldName.startsWith("class$")) {
						if (condition.getOperator().equals("==") && exp1.isBinaryOperatorExpression()
								&& checkFieldReference(fieldName, exp2)) {
							if (exp1.getRightExpression().isMethodInvocationExpression()
									&& checkFieldReference(fieldName, exp1.getLeftExpression())) {
								MethodInvocationExpression mie = (MethodInvocationExpression) exp1.getRightExpression();

								if (mie.getParameters().isStringConstantExpression() && mie.getName().equals("class$")
										&& mie.getInternalTypeName().equals(internalTypeName)) {
									// JDK 1.4.2 '.class' found ==> Convert '(class$java$lang$String == null) ?
									// (class$java$lang$String = TestDotClass.class$("java.lang.String") :
									// class$java$lang$String)' to 'String.class'
									return createObjectTypeReferenceDotClassExpression(lineNumber, fieldName, mie);
								}
							}
						} else if (condition.getOperator().equals("!=") && exp2.isBinaryOperatorExpression()
								&& checkFieldReference(fieldName, exp1)) {
							if (exp2.getRightExpression().isMethodInvocationExpression()
									&& checkFieldReference(fieldName, exp2.getLeftExpression())) {
								MethodInvocationExpression mie = (MethodInvocationExpression) exp2.getRightExpression();

								if (mie.getParameters().isStringConstantExpression() && mie.getName().equals("class$")
										&& mie.getInternalTypeName().equals(internalTypeName)) {
									// JDK 1.1.8 '.class' found ==> Convert '(class$java$lang$String != null) ?
									// class$java$lang$String : (class$java$lang$String =
									// TestDotClass.class$("java.lang.String"))' to 'String.class'
									return createObjectTypeReferenceDotClassExpression(lineNumber, fieldName, mie);
								}
							}
						}
	...
	protected TernaryOperatorExpression newTernaryOperatorExpression(int lineNumber, Expression condition,
			Expression expressionTrue, Expression expressionFalse) {
		...
		if (expressionTrue.isNullExpression()) {
			...
			int flags = ((PrimitiveType) expressionTrueType).getFlags()
					| ((PrimitiveType) expressionFalseType).getFlags();
			...
			if ((flags & FLAG_DOUBLE) != 0) {
				...
				flags = ((PrimitiveType) expressionTrueType).getFlags()
						& ((PrimitiveType) expressionFalseType).getFlags();
	...
	protected Expression createObjectTypeReferenceDotClassExpression(int lineNumber, String fieldName,
			...
			if (statement.getExpression().isPreOperatorExpression()) {
				...
				if ("++".equals(operator) || "--".equals(operator)) {
					if (statement.isExpressionStatement()) {
						...
						es.setExpression(
								new PostOperatorExpression(poe.getLineNumber(), poe.getExpression(), operator));
