...
public class AnnotationConverter implements ElementValueVisitor {
	...
	protected AnnotationReference convert(Annotation annotation) {
		...
		assert (descriptor != null) && (descriptor.length() > 2) && (descriptor.charAt(0) == 'L')
		...
		if (elementValuePairs == null) {
			...
			if ("value".equals(elementName)) {
				...
				return new AnnotationReference(ot, new org.jd.core.v1.model.javasyntax.reference.ElementValuePair(
						elementName, convert(elementValue)));
			}
		} else {
			...
			for (ElementValuePair elementValuePair : elementValuePairs) {
				String elementName = elementValuePair.getElementName();
				ElementValue elementValue = elementValuePair.getElementValue();
				list.add(new org.jd.core.v1.model.javasyntax.reference.ElementValuePair(elementName,
	...
	public void visit(ElementValuePrimitiveType elementValuePrimitiveType) {
		...
		case 'B':
			elementValue = new ExpressionElementValue(new IntegerConstantExpression(PrimitiveType.TYPE_BYTE,
					elementValuePrimitiveType.<ConstantInteger>getConstValue().getValue()));
			break;
		case 'D':
			elementValue = new ExpressionElementValue(
					new DoubleConstantExpression(elementValuePrimitiveType.<ConstantDouble>getConstValue().getValue()));
			break;
		case 'F':
			elementValue = new ExpressionElementValue(
					new FloatConstantExpression(elementValuePrimitiveType.<ConstantFloat>getConstValue().getValue()));
			break;
		case 'I':
			elementValue = new ExpressionElementValue(new IntegerConstantExpression(PrimitiveType.TYPE_INT,
					elementValuePrimitiveType.<ConstantInteger>getConstValue().getValue()));
			break;
		case 'J':
			elementValue = new ExpressionElementValue(
					new LongConstantExpression(elementValuePrimitiveType.<ConstantLong>getConstValue().getValue()));
			break;
		case 'S':
			elementValue = new ExpressionElementValue(new IntegerConstantExpression(PrimitiveType.TYPE_SHORT,
					elementValuePrimitiveType.<ConstantInteger>getConstValue().getValue()));
			break;
		case 'Z':
			elementValue = new ExpressionElementValue(new IntegerConstantExpression(PrimitiveType.TYPE_BOOLEAN,
					elementValuePrimitiveType.<ConstantInteger>getConstValue().getValue()));
			break;
		case 'C':
			elementValue = new ExpressionElementValue(new IntegerConstantExpression(PrimitiveType.TYPE_CHAR,
					elementValuePrimitiveType.<ConstantInteger>getConstValue().getValue()));
			break;
		case 's':
			elementValue = new ExpressionElementValue(
	...
	public void visit(ElementValueEnumConstValue elementValueEnumConstValue) {
		...
		assert (descriptor != null) && (descriptor.length() > 2) && (descriptor.charAt(0) == 'L')
				&& (descriptor.charAt(descriptor.length() - 1) == ';')
				: "AnnotationConverter.visit(elementValueEnumConstValue)";
		...
		elementValue = new ExpressionElementValue(new FieldReferenceExpression(ot,
				new ObjectTypeReferenceExpression(ot), internalTypeName, constName, descriptor));
	}
