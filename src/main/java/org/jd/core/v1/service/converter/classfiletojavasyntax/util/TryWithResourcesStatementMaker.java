...
public class TryWithResourcesStatementMaker {
	...
	public static Statement make(LocalVariableMaker localVariableMaker, Statements statements, Statements tryStatements,
		...
		if (statement.isIfStatement() && (lv1 == getLocalVariable(statement.getCondition()))) {
			...
			if (statement.isIfElseStatement()) {
				return parsePatternAddSuppressed(localVariableMaker, statements, tryStatements, finallyStatements, boe,
						lv1, lv2, statement);
			}
			if (statement.isExpressionStatement()) {
				return parsePatternCloseResource(localVariableMaker, statements, tryStatements, finallyStatements, boe,
						lv1, lv2, statement);
			}
		}

		if (statement.isExpressionStatement()) {
			return parsePatternCloseResource(localVariableMaker, statements, tryStatements, finallyStatements, boe, lv1,
					lv2, statement);
		}
	...
	protected static Statement parsePatternAddSuppressed(LocalVariableMaker localVariableMaker, Statements statements,
			Statements tryStatements, Statements finallyStatements, Expression boe, AbstractLocalVariable lv1,
			AbstractLocalVariable lv2, Statement statement) {
		if (!statement.isIfElseStatement()) {
			return null;
		}
	...
	protected static AbstractLocalVariable getLocalVariable(Expression condition) {
		...
		if (!condition.getOperator().equals("!=") || !condition.getRightExpression().isNullExpression()
				|| !condition.getLeftExpression().isLocalVariableReferenceExpression()) {
			return null;
		}
	...
	protected static Statement parsePatternCloseResource(LocalVariableMaker localVariableMaker, Statements statements,
			Statements tryStatements, Statements finallyStatements, Expression boe, AbstractLocalVariable lv1,
			AbstractLocalVariable lv2, Statement statement) {
		Expression expression = statement.getExpression();
		...
		if (!mie.getName().equals("$closeResource")
				|| !mie.getDescriptor().equals("(Ljava/lang/Throwable;Ljava/lang/AutoCloseable;)V")) {
			return null;
		}
	...
	protected static ClassFileTryStatement newTryStatement(LocalVariableMaker localVariableMaker, Statements statements,
			Statements tryStatements, Statements finallyStatements, Expression boe, AbstractLocalVariable lv1,
