...
public class ControlFlowGraphReducer {
    ...
    protected static boolean reduceConditionalBranch(BasicBlock basicBlock) {
        ...
        if (next.matchType(GROUP_SINGLE_SUCCESSOR | TYPE_RETURN | TYPE_RETURN_VALUE | TYPE_THROW)
            ...
            while ((nextLast != nextNext) && nextNext.matchType(GROUP_SINGLE_SUCCESSOR)
            ...
            if (branch.matchType(GROUP_SINGLE_SUCCESSOR) && (branch.getPredecessors().size() == 1)) {
                ...
                while ((branchLast != branchNext) && branchNext.matchType(GROUP_SINGLE_SUCCESSOR)
                ...
                if (nextNext == branchNext) {
                    ...
                    if ((nextNext.getFromOffset() < branch.getFromOffset())
                            && (nextNext.getPredecessors().size() == 1)) {
                        createIf(basicBlock, next, nextNext, branch);
                        return true;
                    } else if (((nextNext.getFromOffset() > branch.getFromOffset())
                            && branchNext.matchType(GROUP_END))) {
        ...
        if (branch.matchType(GROUP_SINGLE_SUCCESSOR | TYPE_RETURN | TYPE_RETURN_VALUE | TYPE_THROW)
                && (branch.getPredecessors().size() == 1)) {
            BasicBlock branchLast = branch;
            BasicBlock branchNext = branch.getNext();
    ...
    protected static void createIfElse(int type, BasicBlock basicBlock, BasicBlock sub1, BasicBlock last1,
            BasicBlock sub2, BasicBlock last2, BasicBlock next) {
        BasicBlock condition = basicBlock.getControlFlowGraph().newBasicBlock(basicBlock);
    ...
    protected static boolean aggregateConditionalBranches(BasicBlock basicBlock) {
        ...
        if ((next.getType() == TYPE_GOTO_IN_TERNARY_OPERATOR) && (next.getPredecessors().size() == 1)) {
            ...
            if (nextNext.matchType(TYPE_CONDITIONAL_BRANCH | TYPE_CONDITION)) {
                if (branch.matchType(TYPE_STATEMENTS | TYPE_GOTO_IN_TERNARY_OPERATOR) && (nextNext == branch.getNext())
                        && (branch.getPredecessors().size() == 1) && (nextNext.getPredecessors().size() == 2)) {
                    ...
                    if ((nextNextNext.getType() == TYPE_GOTO_IN_TERNARY_OPERATOR)
                        ...
                        if (nextNextNextNext.matchType(TYPE_CONDITIONAL_BRANCH | TYPE_CONDITION)) {
                            if (nextNextBranch.matchType(TYPE_STATEMENTS | TYPE_GOTO_IN_TERNARY_OPERATOR)
                                    && (nextNextNextNext == nextNextBranch.getNext())
                                    && (nextNextBranch.getPredecessors().size() == 1)
                                    && (nextNextNextNext.getPredecessors().size() == 2)) {
            ...
            if ((lineNumber2 - lineNumber1) <= 1) {
                ...
                if (next.matchType(TYPE_CONDITIONAL_BRANCH | GROUP_CONDITION) && (next.getPredecessors().size() == 1)) {
                    ...
                    if (next.getNext() == branch) {
                        ...
                        updateConditionalBranches(basicBlock, createLeftInverseCondition(basicBlock),
                                TYPE_CONDITION_AND, next);
                        return true;
                        ...
                        if (branch.matchType(TYPE_CONDITIONAL_BRANCH | GROUP_CONDITION)) {
                            if ((next.getNext() == branch.getNext()) && (next.getBranch() == branch.getBranch())) {
                                ...
                                return true;
                            } else if ((next.getBranch() == branch.getNext())
                                    && (next.getNext() == branch.getBranch())) {
                                updateConditionTernaryOperator2(basicBlock);
            ...
            if ((lineNumber2 - lineNumber1) <= 1) {
                ...
                if (branch.matchType(TYPE_CONDITIONAL_BRANCH | GROUP_CONDITION)
                    ...
                    if (branch.getBranch() == next) {
                        updateConditionalBranches(basicBlock, createLeftCondition(basicBlock), TYPE_CONDITION_AND,
                                branch);
                        return true;
                    } else if (branch.getNext() == next) {
                        updateConditionalBranches(basicBlock, createLeftInverseCondition(basicBlock), TYPE_CONDITION_OR,
                                branch);
                        return true;
                    }
                }
    ...
    protected static BasicBlock createLeftCondition(BasicBlock basicBlock) {
        if (basicBlock.getType() == TYPE_CONDITIONAL_BRANCH) {
            return basicBlock.getControlFlowGraph().newBasicBlock(TYPE_CONDITION, basicBlock.getFromOffset(),
                    basicBlock.getToOffset(), false);
        } else {
    ...
    protected static BasicBlock createLeftInverseCondition(BasicBlock basicBlock) {
        if (basicBlock.getType() == TYPE_CONDITIONAL_BRANCH) {
            return basicBlock.getControlFlowGraph().newBasicBlock(TYPE_CONDITION, basicBlock.getFromOffset(),
                    basicBlock.getToOffset());
        } else {
            return basicBlock.getControlFlowGraph().newBasicBlock(basicBlock);
        }
    }

    protected static void updateConditionalBranches(BasicBlock basicBlock, BasicBlock leftBasicBlock, int operator,
    ...
    protected static void convertConditionalBranchToGotoInTernaryOperator(BasicBlock basicBlock, BasicBlock next,
            BasicBlock nextNext) {
    ...
    protected static boolean checkJdk118TernaryOperatorPattern(BasicBlock next, BasicBlock nextNext, int ifByteCode) {
        if ((nextNext.getToOffset() - nextNext.getFromOffset()) == 3) {
            ...
            return (code[nextFromOffset] == 3) && // ICONST_0
                    (((code[nextFromOffset + 1] & 255) == 167) || ((code[nextFromOffset + 1] & 255) == 200)) && // GOTO
                                                                                                                // or
                                                                                                                // GOTO_W
    ...
    protected static boolean reduceTryDeclaration(BitSet visited, BasicBlock basicBlock, BitSet jsrTargets) {
        ...
        if (tryWithResourcesFlag) {
            ...
            if ((finallyBB != null) && (basicBlock.getExceptionHandlers().size() == 1) && (tryBB.getType() == TYPE_TRY)
                    && (tryBB.getNext() == END) && (basicBlock.getFromOffset() == tryBB.getFromOffset())
    ...
    protected static boolean checkEclipseFinallyPattern(BasicBlock basicBlock, BasicBlock finallyBB, int maxOffset) {
        ...
        if ((nextOpcode == 0) || (nextOpcode == 167) || // GOTO
    ...
    protected static BasicBlock searchEndBlock(BasicBlock basicBlock, int maxOffset) {
        ...
        if (!last.matchType(GROUP_END)) {
            ...
            if ((next.getFromOffset() >= maxOffset) || (!next.matchType(
                    TYPE_END | TYPE_RETURN | TYPE_SWITCH_BREAK | TYPE_LOOP_START | TYPE_LOOP_CONTINUE | TYPE_LOOP_END)
                    && (next.getToOffset() < basicBlock.getFromOffset()))) {
                return next;
            }
        ...
        for (BasicBlock.ExceptionHandler exceptionHandler : basicBlock.getExceptionHandlers()) {
            ...
            if (bb.getFromOffset() < maxOffset) {
                ...
                if (!last.matchType(GROUP_END)) {
                    ...
                    if ((next.getFromOffset() >= maxOffset) || (!next.matchType(TYPE_END | TYPE_RETURN
                            | TYPE_SWITCH_BREAK | TYPE_LOOP_START | TYPE_LOOP_CONTINUE | TYPE_LOOP_END)
                            && (next.getToOffset() < basicBlock.getFromOffset()))) {
                        return next;
                    }
                ...
                while ((last != next) && last.matchType(GROUP_SINGLE_SUCCESSOR) && (next.getPredecessors().size() == 1)
                        && (lineNumber <= cfg.getLineNumber(next.getFromOffset()))) {
    ...
    protected static boolean reduceJsr(BitSet visited, BasicBlock basicBlock, BitSet jsrTargets) {
        ...
        if (basicBlock.getBranch().getPredecessors().size() > 1) {
            ...
            while (iterator.hasNext()) {
                ...
                if ((predecessor != basicBlock) && (predecessor.getType() == TYPE_JSR)
                        && (predecessor.getNext() == next)) {
    ...
    protected static boolean reduceLoop(BitSet visited, BasicBlock basicBlock, BitSet jsrTargets) {
        ...
        if (reduced == false) {
            ...
            if (updateBasicBlock != null) {
                ...
                BasicBlock ifBasicBlock = basicBlock.getControlFlowGraph().newBasicBlock(TYPE_IF,
                        basicBlock.getSub1().getFromOffset(), basicBlock.getToOffset());
    ...
    protected static BasicBlock searchUpdateBlockAndCreateContinueLoop(BitSet visited, BasicBlock basicBlock) {
        ...
        if (!basicBlock.matchType(GROUP_END) && (visited.get(basicBlock.getIndex()) == false)) {
            ...
            case TYPE_LOOP:
                if (updateBasicBlock == null) {
                    updateBasicBlock = searchUpdateBlockAndCreateContinueLoop(visited, basicBlock,
                            basicBlock.getNext());
                }
                break;
            case TYPE_TRY:
            ...
            case TYPE_TRY_DECLARATION:
                for (BasicBlock.ExceptionHandler exceptionHandler : basicBlock.getExceptionHandlers()) {
                    if (updateBasicBlock == null) {
                        updateBasicBlock = searchUpdateBlockAndCreateContinueLoop(visited, basicBlock,
                                exceptionHandler.getBasicBlock());
                    }
                }
                if (updateBasicBlock == null) {
                    updateBasicBlock = searchUpdateBlockAndCreateContinueLoop(visited, basicBlock,
        ...
        if (subBasicBlock != null) {
            if (basicBlock.getFromOffset() < subBasicBlock.getFromOffset()) {
                ...
                if (basicBlock.getFirstLineNumber() == Expression.UNKNOWN_LINE_NUMBER) {
                    if (subBasicBlock.matchType(GROUP_SINGLE_SUCCESSOR)
                            && (subBasicBlock.getNext().getType() == TYPE_LOOP_START)) {
    ...
    protected static BasicBlock newJumpBasicBlock(BasicBlock bb, BasicBlock target) {
        ...
        return bb.getControlFlowGraph().newBasicBlock(TYPE_JUMP, bb.getFromOffset(), target.getFromOffset(),
                predecessors);
    }

    protected static BasicBlock clone(BasicBlock bb, BasicBlock next) {
        BasicBlock clone = next.getControlFlowGraph().newBasicBlock(next.getType(), next.getFromOffset(),
                next.getToOffset());
        clone.setNext(END);
